/* ===============================================================
   ...
   ABLAUF:
   1. Interne Mail (Tabelle mit allen Daten) → NUR an eldegruen-service@gmx.de
   2. Bestätigungsmail (professionelles Design) → NUR an den Kunden
   =============================================================== */
...
export async function onRequestPost(context) {
  ...
  // Honeypot — Bot erkannt, still ignorieren
  if (honeypot !== '') {
  ...
  // ── 1. INTERNE MAIL → nur an dich (eldegruen-service@gmx.de) ────────────
  // Enthält alle Kundendaten als übersichtliche Tabelle.
  // reply_to = Kunden-E-Mail, damit du direkt antworten kannst.
  const internHtml = `
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: ABSENDER,
        to: [EMPFAENGER],      // ← nur du bekommst diese Mail
        reply_to: email,        // ← Antworten gehen direkt an den Kunden
    ...
    // ── 2. BESTÄTIGUNGSMAIL → nur an den Kunden ──────────────────────────
    // Professionelles Design, KEINE internen Daten (Adresse etc.).
    // reply_to = deine Adresse, damit Kunden-Antworten bei dir ankommen.
    const kundenHtml = `
        ...
        <div style="max-width:540px;margin:0 auto">
          ...
          <div style="background:#ffffff;border-radius:0 0 14px 14px;overflow:hidden;border:1px solid #e2e7de;border-top:none;box-shadow:0 8px 28px rgba(47,82,51,.08)">
            ...
            <div style="padding:36px 34px 30px;color:#222;font-size:15px;line-height:1.7">
              ...
              <p style="margin-bottom:0">Mit freundlichen Grüßen<br><strong>Ihr ELDEGRUEN-Team</strong></p>
            </div>
            <div style="background:#f7f8f5;border-top:1px solid #eceee9;padding:18px 30px;text-align:center;font-size:11.5px;color:#9a9a9a;letter-spacing:.3px">
              An der Domsühler Straße 2 · 19374 Domsühl<br>
              eldegruen-service.de &nbsp;·&nbsp; ${EMPFAENGER}
            </div>
          </div>
        </div>
      </div>
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: ABSENDER,
          to: [email],           // ← nur der Kunde bekommt diese Mail
          reply_to: EMPFAENGER,  // ← Antworten gehen zu dir
