...
//
// WICHTIG (Schutzbereich): Nur die unten in PROTECTED_PATHS gelisteten Seiten
// sind passwortgeschützt. Die normale Hauptseite (index.html / "/") und alle
// anderen Dateien (Bilder, CSS, JS, ...) werden IMMER frei ausgeliefert.

const COOKIE_NAME = "eg_session";
const SESSION_HOURS = 12;
const NO_STORE = { "Cache-Control": "no-store, no-cache, must-revalidate", "Pragma": "no-cache" };

// Pfade, die durch das Passwort geschützt werden sollen.
// Hier ggf. weitere geschützte Seiten ergänzen (jeweils der Pfad, wie er
// im Browser aufgerufen wird).
const PROTECTED_PATHS = [
  "/Unternehmerverwaltung.html",
  "/Unternehmerverwaltung", // falls die Seite ohne .html aufgerufen wird
];

function isProtectedPath(pathname) {
  return PROTECTED_PATHS.some(
    (p) => pathname === p || pathname === p.toLowerCase() || pathname.toLowerCase() === p.toLowerCase()
  );
}
...
function loginPage(error, misconfigured, redirectTo) {
  const safeRedirect = redirectTo && redirectTo.startsWith("/") ? redirectTo : "/Unternehmerverwaltung.html";
  return `<!DOCTYPE html>
...
export async function onRequest(context) {
  ...
  // Login-Verarbeitung (die Login-Seite selbst bleibt natürlich frei erreichbar)
  if (url.pathname === "/login" && request.method === "POST") {
    ...
    const redirectTo = (form.get("redirect") || "/Unternehmerverwaltung.html").toString();
    const safeRedirect = redirectTo.startsWith("/") ? redirectTo : "/Unternehmerverwaltung.html";
    ...
    return new Response(loginPage(true, false, safeRedirect), { status: 401, headers: { "Content-Type": "text/html; charset=UTF-8", ...NO_STORE } });
  }

  // Nur geschützte Pfade werden überhaupt geprüft. Alles andere (Hauptseite,
  // Bilder, CSS/JS, etc.) läuft ohne jede Passwortprüfung direkt durch.
  if (!isProtectedPath(url.pathname)) {
    return next();
  }

  if (!secretsConfigured) {
    return new Response(loginPage(false, true, url.pathname), { status: 500, headers: { "Content-Type": "text/html; charset=UTF-8", ...NO_STORE } });
  }

  // Geschützter Pfad: Session prüfen
  if (await isValidSession(request, env)) {
    return next(); // Passwort korrekt -> Seite normal ausliefern
  }

  return new Response(loginPage(false, false, url.pathname), { status: 401, headers: { "Content-Type": "text/html; charset=UTF-8", ...NO_STORE } });
}
